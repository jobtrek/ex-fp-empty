...
export function populationDensityRanking(): CountryDensity[] {
  ...
  return countries
    .filter((c) => c.area > 0)
    .map((c) => ({ name: c.name.common, density: c.population / c.area }))
...
export function populationByRegion(): RegionPopulation[] {
  ...
  // --sw-wipe--
  const totals = countries.reduce<Record<string, number>>((acc, c) => {
    acc[c.region] = (acc[c.region] ?? 0) + c.population;
    return acc;
  }, {});

  return Object.entries(totals)
    .map(([region, totalPopulation]) => ({ region, totalPopulation }))
...
export function countryPopulationLabel(country: Country): string {
  ...
  // --sw-wipe--
  const name = extractName(country);
  const millions = toMillions(country.population);
  return formatLabel(name)(millions);
  // --sw-wipe--
}
...
export function largestCountryPerRegion(): RegionLargest[] {
  ...
  // --sw-wipe--
  const largest = countries.reduce<Record<string, Country>>((acc, c) => {
    if (!acc[c.region] || c.area > (acc[c.region]?.area ?? 0)) {
      acc[c.region] = c;
    }
    return acc;
  }, {});
  ...
  return Object.entries(largest)
    .map(([region, c]) => ({ region, country: c.name.common, area: c.area }))
